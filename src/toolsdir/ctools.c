...
void copy ( register char *dest, register char *src, register int n)
...
int to_upper_if_lower (char ch)

  { return(islower(ch) ? toupper(ch) : ch); }


int to_lower_if_upper (char ch)
...
char *strsearch(register char *s1, register char *s2, register int s1len, register int s2len)
...
char *strncsearch(register char *s1, register char *s2, register int s1len, register int s2len)
...
void string_upcase (char *astring)
{
  while (*astring) {
...
void string_downcase (char *astring)
{
  while (*astring) {
...

{
  ...
  while (T) {
    ...
    switch (len = mygetline(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int mygetline (FILE *iport, char *buffer, int buflen)
...

{
  int len;
  char *line;
  if (0 > (len = mygetline(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
...

{
  ...
  while (T) {
    ...
    if (0 > (len = mygetline(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
